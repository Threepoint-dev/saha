...
public class EventPackageController {
    ...
    private final EventPackageService service;

    public EventPackageController(EventPackageService service) {
        this.service = service;
    }

    @GetMapping
    public List<PackageDto> list(@PathVariable UUID tenantId) {
        return service.listActive(tenantId);
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public PackageDto create(@PathVariable UUID tenantId, @Valid @RequestBody PackageUpsertRequest request) {
        return service.create(tenantId, request);
    }

    @PutMapping("/{id}")
    public PackageDto update(@PathVariable UUID tenantId, @PathVariable UUID id,
